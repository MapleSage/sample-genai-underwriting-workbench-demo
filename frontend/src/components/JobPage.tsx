...
): Record<string, React.FC<MarkdownComponentProps>> => {
  ...
  for (const cssClassOrTag in styles) {
    ...
    tags.forEach((tagString) => {
      ...
      components[Element] = ({ node, children, ...props }) => {
        return (
          <Element style={styleObject} {...props}>
            {children}
          </Element>
        );
      };
...
const customMarkdownComponentsFromStyles =
  generateMarkdownComponents(markdownStyles);
...
const PageContext = createContext<[number, (page: number) => void]>([
  1,
  () => {},
]);
const NumPagesContext = createContext<
  [number | null, (pages: number | null) => void]
>([null, () => {}]);

const getDocumentIcon = (documentType: string) => {
  if (!documentType) return faFileAlt;
  const lowerDocType = documentType.toLowerCase();
  if (/medic(al|ation)|(health|disease)/i.test(lowerDocType))
    return faFileMedical;
  if (/history|anamnesis/i.test(lowerDocType)) return faHistory;
  if (/pharmac(y|eutical)|medication|drug|prescription/i.test(lowerDocType))
    return faPills;
  if (/lab(oratory)?|clinical|test|specimen/i.test(lowerDocType))
    return faFlask;
  if (/physician|doctor|practitioner|clinician|md\b/i.test(lowerDocType))
    return faUserMd;
  if (/exam(ination)?|assessment|paramedical/i.test(lowerDocType))
    return faStethoscope;
  if (/hospital|clinic|center|facility|institution/i.test(lowerDocType))
    return faHospital;
  if (/x-ray|imaging|scan|radiolog(y|ical)|mri|ct scan/i.test(lowerDocType))
    return faXRay;
  if (/surg(ery|ical)|procedure|operation/i.test(lowerDocType))
    return faProcedures;
  if (/cardio|heart|cardiac|pulse|ekg|ecg/i.test(lowerDocType))
    return faHeartbeat;
  ...
  if (/eye|vision|ophthalm(ology|ologist)|optical/i.test(lowerDocType))
    return faEye;
  if (
    /insurance|financial|coverage|policy|premium|underwriter/i.test(
      lowerDocType
    )
  )
    return faFileInvoiceDollar;
  if (/form|(question|survey)(naire)?|assessment/i.test(lowerDocType))
    return faClipboardList;
  if (/note|report|summary|record/i.test(lowerDocType)) return faNotesMedical;
  if (
    /microscop(e|ic)|patholog(y|ical)|cytolog(y|ical)|histolog(y|ical)/i.test(
      lowerDocType
    )
  )
    return faMicroscope;
  if (/blood|hematolog(y|ical)|serum|plasma|specimen/i.test(lowerDocType))
    return faVial;
  if (/emergency|urgent|trauma|ambulance|ems/i.test(lowerDocType))
    return faBriefcaseMedical;
  if (
    /home|property|dwelling|real estate|building|structure/i.test(lowerDocType)
  )
    return faHome;
  if (/auto|car|vehicle|motorcycle|truck|collision/i.test(lowerDocType))
    return faCar;
  if (
    /commercial|business property|office|warehouse|retail/i.test(lowerDocType)
  )
    return faBuilding;
  if (/umbrella|liability|excess|protection/i.test(lowerDocType))
    return faUmbrella;
  if (/flood|water damage|rising water|overflow/i.test(lowerDocType))
    return faWater;
  if (/fire|flame|burn|combustion|smoke/i.test(lowerDocType)) return faFire;
  if (/legal|liability|lawsuit|litigation|tort/i.test(lowerDocType))
    return faBalanceScale;
  if (/hazard|risk|danger|peril|warning/i.test(lowerDocType))
    return faExclamationTriangle;
  if (/fleet|commercial auto|commercial vehicle|transport/i.test(lowerDocType))
    return faTruck;
  if (
    /workers comp|workers' compensation|workplace injury|occupational/i.test(
      lowerDocType
    )
  )
    return faHardHat;
  if (/industrial|manufacturing|factory|plant|production/i.test(lowerDocType))
    return faIndustry;
  if (/storm|hurricane|tornado|hail|weather damage/i.test(lowerDocType))
    return faCloudShowersHeavy;
  if (/wind|windstorm|gust|gale/i.test(lowerDocType)) return faWind;
  if (/roadway|highway|traffic|intersection|accident|crash/i.test(lowerDocType))
    return faRoad;
  if (/protection|security|safeguard|defense|safety/i.test(lowerDocType))
    return faShieldAlt;
  if (/claim|judgment|settlement|adjudication|ruling/i.test(lowerDocType))
    return faGavel;
  if (/contract|agreement|terms|certificate/i.test(lowerDocType))
...
const STATUS_MAPPING = {
  CREATED: {
    ...
    phase: "Job Created",
    details:
      "Job has been created and is waiting for document upload to complete...",
  },
  UPLOAD_PENDING: {
    step: 1,
  ...
  CLASSIFYING: {
    ...
    details:
      "The AI is analyzing the document to determine its type and structure for optimal processing...",
  },
  EXTRACTING: {
    step: 2,
    phase: "Extracting Information",
    details:
      "Advanced AI models are reading through the document and extracting key information and data points...",
  },
  ANALYZING: {
    step: 3,
    phase: "Analyzing Content",
    details:
      "Our underwriting AI is performing comprehensive analysis to identify risks, discrepancies, and insights...",
  },
  ACTING: {
    step: 4,
    phase: "Taking Action",
  ...
  COMPLETE: {
    ...
    phase: "Complete",
    details:
      "Analysis complete! Review the detailed results and AI recommendations below.",
  },
  Failed: {
    step: 5,
    phase: "Failed",
...
export function JobPage({ jobId }: JobPageProps) {
  ...
  const [insuranceType, setInsuranceTypeState] = useState<
  ...
  const [agentActionData, setAgentActionData] =
    useState<AgentActionData | null>(null);
  ...
  const fetchJobDetailsAndUpdateState = useCallback(
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const errorMsg =
            ...
            errorData.message ||
            errorData.error ||
            `Failed to fetch job details: ${response.status}`;
        ...
        if (jobApiData.extractedData) {
          try {
            console.log(
              "Using directly provided extractedData",
              jobApiData.extractedData
            );
        ...
        // Then fall back to extractedDataJsonStr if needed
          ...
          try {
            const parsedExtraction = JSON.parse(
            ...
            if (
              ...
              for (const key in parsedExtraction) {
                if (
                  Object.prototype.hasOwnProperty.call(parsedExtraction, key)
                ) {
              ...
              console.warn(
                "extractedDataJsonStr did not parse to a valid object."
              );
          ...
          jobApiData.extracted_data?.sections &&
          jobApiData.extracted_data.sections.length > 0
        ) {
            ...
            if (section.findings && section.findings.length > 0) {
              content +=
                ...
                section.findings
                  .map(
                    (f) =>
                      `- ${f.type}: ${f.description} (Severity: ${
                        f.severity || "N/A"
                      })${
                        f.page_references && f.page_references.length > 0
                          ? ` (Pages: ${f.page_references.join(", ")})`
                          : ""
                      }`
                  )
                  .join("\n");
          ...
          console.log(
            "Populated page_analysis from extracted_data.sections (fallback)"
          );
        } else {
          console.log(
            "No data available to populate page_analysis for Document Analysis tab."
          );
        ...
        if (jobApiData.analysisOutput) {
          console.log(
            "Using directly provided analysisOutput",
            jobApiData.analysisOutput
          );
          try {
            // Transform identified_risks array to markdown string
            const riskAssessment =
              jobApiData.analysisOutput.identified_risks &&
              Array.isArray(jobApiData.analysisOutput.identified_risks)
                ? jobApiData.analysisOutput.identified_risks
                    .map((risk: any) => {
                      const pageRefs =
                        risk.page_references &&
                        Array.isArray(risk.page_references) &&
                        risk.page_references.length > 0
                          ? ` ([${risk.page_references.join(", ")}](/page/${
                              risk.page_references[0]
                            }))`
                          : "";
                      return `- **${risk.severity || "N/A"}**: ${
            ...
            const discrepancies =
              jobApiData.analysisOutput.discrepancies &&
              Array.isArray(jobApiData.analysisOutput.discrepancies)
                ? jobApiData.analysisOutput.discrepancies
                    .map((disc: any) => {
                      const pageRefs =
                        disc.page_references &&
                        Array.isArray(disc.page_references) &&
                        disc.page_references.length > 0
                          ? ` ([${disc.page_references.join(", ")}](/page/${
                              disc.page_references[0]
                            }))`
                          : "";
            ...
            underwriterAnalysisTransformed = {
              ...
              MEDICAL_TIMELINE:
                jobApiData.insurance_type === "life"
                  ? jobApiData.analysisOutput.medical_timeline ||
                    "Not available."
                  : undefined,
              PROPERTY_ASSESSMENT:
                jobApiData.insurance_type === "property_casualty"
                  ? jobApiData.analysisOutput.property_assessment ||
                    "Not available."
                  : undefined,
              FINAL_RECOMMENDATION:
                jobApiData.analysisOutput.final_recommendation ||
                "Not available.",
            };
        ...
        // Then try to parse the analysisOutputJsonStr field
          ...
          try {
            ...
            // Transform identified_risks array to markdown string
            const riskAssessment =
              parsedAnalysis.identified_risks &&
              Array.isArray(parsedAnalysis.identified_risks)
                ? parsedAnalysis.identified_risks
                    .map((risk: any) => {
                      const pageRefs =
                        risk.page_references &&
            ...
            // Transform discrepancies array to markdown string
            const discrepancies =
              parsedAnalysis.discrepancies &&
              Array.isArray(parsedAnalysis.discrepancies)
                ? parsedAnalysis.discrepancies
                    .map((disc: any) => {
                      const pageRefs =
                        disc.page_references &&
            ...
            underwriterAnalysisTransformed = {
              ...
              MEDICAL_TIMELINE:
                jobApiData.insurance_type === "life"
                  ? parsedAnalysis.medical_timeline || "Not available."
                  : undefined,
              PROPERTY_ASSESSMENT:
                jobApiData.insurance_type === "property_casualty"
                  ? parsedAnalysis.property_assessment || "Not available."
                  : undefined,
              FINAL_RECOMMENDATION:
                parsedAnalysis.final_recommendation || "Not available.",
            };
          } catch (parseError) {
            ...
            underwriterAnalysisTransformed = {
              RISK_ASSESSMENT: jobApiData.identified_risks
                ? jobApiData.identified_risks
                    .map(
                      (r) =>
                        `- ${r.description} (Severity: ${r.severity || "N/A"})${
                          r.page_references && r.page_references.length > 0
                            ? ` (Pages: ${r.page_references.join(", ")})`
                            : ""
                        }`
                    )
                    .join("\n")
                : "Not available.",
              DISCREPANCIES:
                jobApiData.discrepancies_summary || "Not available.",
              MEDICAL_TIMELINE:
                jobApiData.insurance_type === "life"
                  ? jobApiData.medical_timeline_summary || "Not available."
                  : undefined,
              PROPERTY_ASSESSMENT:
                jobApiData.insurance_type === "property_casualty"
                  ? jobApiData.property_assessment_summary || "Not available."
                  : undefined,
              FINAL_RECOMMENDATION:
                jobApiData.underwriting_recommendation || "Not available.",
            };
          }
        } else {
        ...
        if (jobApiData.agentActionOutput) {
          console.log(
            "Found direct agentActionOutput:",
            jobApiData.agentActionOutput
          );

          // Only show popup if we don't already have this agent action data (to avoid showing on every poll)
          if (
            !agentActionData ||
            JSON.stringify(agentActionData) !==
              JSON.stringify(jobApiData.agentActionOutput)
          ) {
            setAgentActionData(jobApiData.agentActionOutput);
            setShowAgentActionPopup(true);
            console.log(
              "Agent action popup will be shown from direct agentActionOutput"
            );
          }
        }
        // Then fall back to parsing agentActionOutputJsonStr if needed
          ...
          try {
            const parsedAgentAction: AgentActionData = JSON.parse(
              jobApiData.agentActionOutputJsonStr
            );
            console.log(
              "Parsed agent action data from JSON string:",
              parsedAgentAction
            );
            ...
            if (
              !agentActionData ||
              JSON.stringify(agentActionData) !==
                JSON.stringify(parsedAgentAction)
            ) {
              setAgentActionData(parsedAgentAction);
              setShowAgentActionPopup(true);
              console.log(
                "Agent action popup will be shown from parsed agentActionOutputJsonStr"
              );
            }
          } catch (parseError) {
            console.error(
              "Error parsing agentActionOutputJsonStr:",
              parseError
            );
          }
        }
        ...
        if (statusInfo) {
          ...
          if (statusKey === "COMPLETE") {
            ...
            setErrorOriginal(
              jobApiData.error_message || "Job processing failed."
            );
            setShowError(true);
            if (pollingIntervalRef.current) {
            ...
            if (!pollingIntervalRef.current) {
              console.log(
                `Job status is ${statusKey} - starting polling every 5 seconds`
              );
              pollingIntervalRef.current = setInterval(
                () => fetchJobDetailsAndUpdateState(true),
                5000
              );
            }
          }
          ...
          if (!pollingIntervalRef.current) {
            console.log(
              `Unknown status ${jobApiData.status} - starting polling every 5 seconds`
            );
            pollingIntervalRef.current = setInterval(
              () => fetchJobDetailsAndUpdateState(true),
              5000
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // No specific cleanup needed for direct Azure presigned URLs as they are not blob URLs
  ...
  useEffect(() => {
    if (analysisData?.insurance_type) {
      ...
      let greeting =
        "Hi! I'm your AI assistant. I've analyzed this document and can help answer any questions you have about it.";
      if (type === "property_casualty") {
        greeting =
          "Hello! I'm your Property & Casualty insurance underwriting assistant. I've analyzed this document and can help with questions about property details, risk factors, and coverage considerations.";
      } else if (type === "life") {
        greeting =
          "Hello! I'm your Life Insurance underwriting assistant. I've analyzed this document and can help with questions about medical history, risk factors, and policy considerations.";
      }
      setMessages([
      ...
      setMessages([
        {
          id: "1",
          text: "Hi! I'm your AI assistant. How can I help with this document?",
          sender: "ai",
          timestamp: new Date(),
        },
      ]);
  ...
  const renderGroupedAnalysis = () => {
    ...
    if (
      !localAnalysisData?.page_analysis ||
      Object.keys(localAnalysisData.page_analysis).length === 0
    ) {
      if (isLoadingJobDetails || currentPhase !== "Complete")
        return <p>Loading document analysis sections...</p>;
      return <p>No document page analysis data available.</p>;
    }

    const pagesToRender = Object.entries(localAnalysisData.page_analysis).map(
      ([pageNumKey, pageDataVal]) => ({
        pageNumKey,
        pageType: pageDataVal.page_type,
        content: pageDataVal.content,
        numericPageNum:
          pageDataVal.numeric_page_num_for_nav ||
          parseInt(pageNumKey.replace(/[^0-9]/g, "")) ||
          1,
      })
    );
    ...
    return (
      <div className="grouped-analysis">
        {Object.entries(groups).map(([groupTitle, groupPages]) => {
          const sortedPages = [...groupPages].sort(
            (a, b) => a.numericPageNum - b.numericPageNum
          );
            ...
            <div key={groupTitle} className={`analysis-group`}>
              <button
                className={`group-header ${
                  expandedGroups.has(groupTitle) ? "expanded" : ""
                }`}
                onClick={() => {
                ...
                <div className="group-content">
                  {groupPages.map((page) => (
                    <div
                      key={page.pageNumKey}
                      className={`page-card ${
                        currentPage === page.numericPageNum ? "active" : ""
                      }`}
                      ...
                      <ReactMarkdown
                        ...
                        components={customMarkdownComponentsFromStyles}>
                        {page.content}
                      </ReactMarkdown>
                    </div>
                  ))}
  ...
  const renderUnderwriterAnalysis = () => {
    ...
    if (!localAnalysisData?.underwriter_analysis) {
      if (isLoadingJobDetails || currentPhase !== "Complete")
        return <p>Loading underwriter analysis...</p>;
    ...
    const sectionConfig: SectionConfigItem[] =
      localAnalysisData.insurance_type === "property_casualty"
        ? [
            {
              key: "RISK_ASSESSMENT",
    ...
    return (
      <div className="underwriter-analysis">
        {sectionConfig.map((item) => {
          ...
          if (
            item.key === "MEDICAL_TIMELINE" &&
            localAnalysisData.insurance_type !== "life"
          )
            return null;
          if (
            item.key === "PROPERTY_ASSESSMENT" &&
            localAnalysisData.insurance_type !== "property_casualty"
          )
            return null;
            ...
            <div key={item.key} className="analysis-section">
              ...
              <div className="analysis-content">
                <ReactMarkdown
                  remarkPlugins={[remarkGfm]}
                  components={{
                    ...customMarkdownComponentsFromStyles,
                    a: ({ href, children }) => (
                      <PageReference
                        pageNum={href?.replace("/page/", "") || "1"}
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          id: (Date.now() + 1).toString(),
          text: "Chat error. Please try again.",
          sender: "ai",
          timestamp: new Date(),
        },
      ]);
            ...
            <div className="job-title-section">
              <h1>
                {analysisData?.filename ||
                  `Processing Job ${jobId.slice(0, 8)}...`}
              </h1>
            </div>
            <div className="header-controls">
              {documentType && (
                ...
                <div className="insurance-type-badge">
                  {analysisData.insurance_type === "property_casualty" ? (
                    ...
                    <span className="badge life">
                      <FontAwesomeIcon icon={faBriefcaseMedical} /> Life
                      Insurance
                    </span>
                  )}
                </div>
              ...
              <div className="agent-action-content">
                ...
                <div className="agent-action-header">
                  ...
                  <div className="agent-action-title">
                    ...
                    <p>
                      Our AI agent has processed your document and taken action
                    </p>
                  </div>
                </div>
                <div className="agent-action-body">
                  <div className="agent-action-summary">
                    ...
                    <div className="action-text">
                      {agentActionData.agent_action_confirmation.includes(
                        "email"
                      ) ? (
                        <span>
                          <strong>Email Sent!</strong> The agent has sent an
                          email based on the document analysis.
                        </span>
                      ) : agentActionData.agent_action_confirmation.includes(
                          "ineligible"
                        ) ? (
                        <span>
                          <strong>Application Reviewed:</strong> The agent has
                          determined this application requires review.
                        </span>
                      ) : (
                        <span>
                          <strong>Action Taken:</strong> The agent has completed
                  ...
                  <details className="agent-action-details">
                    ...
                    <div className="action-confirmation">
                      <ReactMarkdown>
                        {agentActionData.agent_action_confirmation}
                      </ReactMarkdown>
                    </div>
                  </details>
                </div>
                ...
                <div className="progress-steps">
                  <div
                    ...
                    {currentStep >= 1 && (
                      <FontAwesomeIcon
                        icon={
                          currentPhase === "Upload Pending"
                            ? faHourglassHalf
                            : currentPhase === "Classifying Document"
                            ? faDatabase
                            : faUpload
                        }
                  ...
                  <div
                    ...
                    {currentStep >= 2 && (
                      <FontAwesomeIcon
                        icon={
                          currentPhase === "Extracting Information"
                            ? faSpinner
                            : faFileAlt
                        }
                        className="progress-icon"
                        spin={currentPhase === "Extracting Information"}
                      />
                    )}
                  </div>
                  <div
                  ...
                  <div
                    ...
                    {currentStep >= 3 && (
                      <FontAwesomeIcon
                        icon={
                          currentPhase === "Analyzing Content"
                            ? faSpinner
                            : faClipboardCheck
                        }
                        className="progress-icon"
                        spin={currentPhase === "Analyzing Content"}
                      />
                    )}
                  </div>
                  <div
                  ...
                  <div
                    ...
                    {currentStep >= 4 && (
                      <FontAwesomeIcon
                        icon={
                          currentPhase === "Taking Action" ? faSpinner : faRobot
                        }
                        className="progress-icon"
                        spin={currentPhase === "Taking Action"}
                      />
                    )}
                  </div>
                  <div
          ...
          {analysisData && (
            <div
              ...
              <Split
                  ...
                  {!pdfDownloadUrl && !isFetchingPdfUrl && (
                    <div className="pdf-placeholder">
                      ...
                      <p className="placeholder-text">
                        PDF document will appear here once analysis is complete.
                      </p>
                    </div>
                  )}
                  {pdfDownloadUrl && (
                    <>
                      <div className="pdf-controls pdf-controls-absolute">
                        <button
                          onClick={() =>
                            setCurrentPageState((p) => Math.max(1, p - 1))
                          }
                        ...
                        <div className="zoom-controls">
                          ...
                          <span className="zoom-level">{`${Math.round(
                            scale * 100
                          )}%`}</span>
                          <button
                            onClick={() =>
                        ...
                        <button
                          onClick={() => fetchJobDetailsAndUpdateState()}
                          title="Refresh Data">
                          ...
                          <button
                            onClick={() => {
                              if (pdfBlob) {
                                ...
                                window.open(pdfDownloadUrl)?.print();
                              }
                            }}
                            title="Print PDF">
                      ...
                      <Document
                        ...
                        error={
                          <div className="pdf-loading">Error loading PDF.</div>
                        }>
                ...
                <div
                  className={`analysis-viewer ${
                    !isAnalysisPanelOpen ? "collapsed" : ""
                  }`}>
                  <div className="tabs">
                    <button
              ...
              <button
                ...
                <FontAwesomeIcon
                  icon={isAnalysisPanelOpen ? faChevronLeft : faChevronRight}
                />
              </button>
            </div>
          )}
